...
class MRSP3():
    ...
    def getFileName(param_dict):
        return 'MRSP3_ENN-{}_Imb-{}_Merging-{}'.format(param_dict['ENN'],\
...
if __name__ == '__main__':
    ...
    X_red_imbFalse_NoENN, y_red_imbFalse_NoENN = MRSP3().reduceSet(X_train_sc, y_train, params_dict)

    params_dict = MRSP3.getParameterDictionary()
    params_dict['imb'] = 'ExcludingSamples'
    params_dict['ENN'] = False
    params_dict['merging'] = 'Base'
    X_red_imbTrue_NoENN, y_red_imbTrue_NoENN = MRSP3().reduceSet(X_train_sc, y_train, params_dict)

    params_dict = MRSP3.getParameterDictionary()
    params_dict['imb'] = 'ExcludingSamples'
    params_dict['ENN'] = True
    params_dict['merging'] = 'Base'
    X_red_imbTrue_YesENN, y_red_imbTrue_YesENN = MRSP3().reduceSet(X_train_sc, y_train, params_dict)

    cls_ori = MLkNN(k=1).fit(X_train, y_train)
    cls_red_imbFalse_NoENN = MLkNN(k=1).fit(X_red_imbFalse_NoENN, y_red_imbFalse_NoENN)
    cls_red_imbTrue_NoENN = MLkNN(k=1).fit(X_red_imbTrue_NoENN, y_red_imbTrue_NoENN)
    cls_red_imbTrue_YesENN = MLkNN(k=1).fit(X_red_imbTrue_YesENN, y_red_imbTrue_YesENN)

    y_pred_ori = cls_ori.predict(X_test_sc)
    y_pred_red_imbFalse_NoENN = cls_red_imbFalse_NoENN.predict(X_test_sc)
    y_pred_red_imbTrue_NoENN = cls_red_imbTrue_NoENN.predict(X_test_sc)
    y_pred_red_imbTrue_YesENN = cls_red_imbTrue_YesENN.predict(X_test_sc)


    print("Results:")
    print("\t - Hamming Loss (init): {:.3f} - Size: {:.1f}%".format(hamming_loss(y_test, y_pred_ori), 100*X_train.shape[0]/X_train.shape[0]))
    print("\t - Hamming Loss (imb-None;MLeNN-Fase): {:.3f} - Size: {:.1f}%".format(hamming_loss(y_test, y_pred_red_imbFalse_NoENN), 100*X_red_imbFalse_NoENN.shape[0]/X_train.shape[0]))
    print("\t - Hamming Loss (imb-ExclSamples;MLeNN-False): {:.3f} - Size: {:.1f}%".format(hamming_loss(y_test, y_pred_red_imbTrue_NoENN), 100*X_red_imbTrue_NoENN.shape[0]/X_train.shape[0]))
    print("\t - Hamming Loss (imb-ExclSamples;MLeNN-True): {:.3f} - Size: {:.1f}%".format(hamming_loss(y_test, y_pred_red_imbTrue_YesENN), 100*X_red_imbTrue_YesENN.shape[0]/X_train.shape[0]))
    print("\t" + "---" * 20)
    ...
    print("\t - F1 (imb-None;MLeNN-Fase): {:.3f} - Size: {:.1f}%".format(f1_score(y_true = y_test, y_pred = y_pred_red_imbFalse_NoENN, average = 'macro'), 100*X_red_imbFalse_NoENN.shape[0]/X_train.shape[0]))
    print("\t - F1 (imb-ExclSamples;MLeNN-False): {:.3f} - Size: {:.1f}%".format(f1_score(y_true = y_test, y_pred = y_pred_red_imbTrue_NoENN, average = 'macro'), 100*X_red_imbTrue_NoENN.shape[0]/X_train.shape[0]))
    print("\t - F1 (imb-ExclSamples;MLeNN-True): {:.3f} - Size: {:.1f}%".format(f1_score(y_true = y_test, y_pred = y_pred_red_imbTrue_YesENN, average = 'macro'), 100*X_red_imbTrue_YesENN.shape[0]/X_train.shape[0]))
    print("Done!")
