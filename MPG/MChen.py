...
class MChen():
    ...
    def getFileName(param_dict):
        return 'MChen_ENN-{}_Imb-{}_Merging-{}_Red-{}'.format(param_dict['ENN'],\
            param_dict['imb'], param_dict['merging'], param_dict['red'])
...
if __name__ == '__main__':
    ...
    X_red_imbFalse_NoENN, y_red_imbFalse_NoENN = MChen().reduceSet(X_train.toarray().copy(), y_train.toarray().copy(), params_dict)

    params_dict = MChen.getParameterDictionary()
    params_dict['imb'] = 'ExcludingSamples'
    params_dict['ENN'] = False
    X_red_imbTrue_NoENN, y_red_imbTrue_NoENN = MChen().reduceSet(X_train.toarray().copy(), y_train.toarray().copy(), params_dict)

    params_dict = MChen.getParameterDictionary()
    params_dict['imb'] = 'ExcludingSamples'
    params_dict['ENN'] = True
    X_red_imbTrue_YesENN, y_red_imbTrue_YesENN = MChen().reduceSet(X_train.toarray().copy(), y_train.toarray().copy(), params_dict)
    ...
    print("\t - Hamming Loss (imb-None;MLeNN-False): {:.3f} - Size: {:.1f}%".format(hamming_loss(y_test, y_pred_red_imbFalse_NoENN), 100*X_red_imbFalse_NoENN.shape[0]/X_train.shape[0]))
    ...
    print("\t - F1 (imb-None;MLeNN-False): {:.3f} - Size: {:.1f}%".format(f1_score(y_true = y_test, y_pred = y_pred_red_imbTrue_NoENN, average = 'macro'), 100*X_red_imbFalse_NoENN.shape[0]/X_train.shape[0]))
